        ...
        if (PostImg == 'ok') {
            ...
            const blob = await new Promise((resolve, reject) => {
                ...
                const snapshot = await ref.put(blob).then(() =>{
                    blob.close();
                    console.log('done.');
                    const res = db.collection('posts').doc(r).set({owner:usernaame,postImg:r,postCap:PostCaption,ownerPic:userLogo,createdAt: timestamp()});
                    let usrPost = [];
                    usrPost.push(r);
      ...
      <View style={{backgroundColor:'#ffffff',paddingBottom:100}} >
          ...
          <TouchableOpacity style={{alignSelf:'center', marginTop:25,backgroundColor:'#ffffff',paddingHorizontal:30,paddingTop:7,borderRadius:150,borderWidth:2,borderRadius:120,borderColor:mcolor}} onPress={pickImage}>
            ...
            <View style={{marginTop:30,backgroundColor:mcolor,paddingTop:5,borderRadius:15,}}>
                ...
                <Image style={{width:40,height:40,borderRadius:100,marginLeft:15,marginTop:5,marginBottom:5,borderWidth:2,borderColor:'#ffffff'}} source={{uri:'https://firebasestorage.googleapis.com/v0/b/pika-4f9d6.appspot.com/o/'+userLogo+'?alt=media&token=63a74aae-9e63-4382-a1fa-9179491c5f16'}} />
