...
import { Animated,StyleSheet, Text, View,Image, TouchableOpacity, TextInput,SafeAreaView,ScrollView,RefreshControl, TouchableWithoutFeedback,ActivityIndicator } from 'react-native';
...
function home_s({navigation}){
  ...
  const onAStart = async() => {
    ...
    if (verifiedd.includes(doc.data().owner)) {
      OnePost.push('verified')
      console.log('verified for: ',doc.data().owner)
      console.log(OnePost)
    }
    likePost.push(OnePost);
    AllPosts.push(OnePost);
  ...
  if (AllPosts1[1]) {
    return(
      <SafeAreaView style={{backgroundColor:bgc,height:'100%'}}>
        <ScrollView showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={[fgc]} progressBackgroundColor={bgc} />}>
          <View>
              <View style={{marginTop:20,marginBottom:10}} >
                  <Text style={{alignSelf:'center',color:fgc,fontSize:34,fontFamily:'cour'}}>pika</Text>
              </View>
              <View style={{marginBottom:57}}>
                  {
                    AllPosts1.map((index,idx)=>{
                      let urdumb = false;
                      let urreallydumb = false;
                      if (idx == 0) {
                        urreallydumb = true
                      }else{
                        urreallydumb = false
                      }
                      if (idx == AllPosts1.length-1) {
                        urdumb = true
                      }else{
                        urdumb = false
                      }
                      return(
                        <View key={index}>
                              <View style={{backgroundColor:fgc,paddingTop:5,marginBottom:1,borderTopLeftRadius: urdumb? 15:0,borderTopRightRadius:urdumb? 15:0,borderBottomLeftRadius:urreallydumb? 15:0,borderBottomRightRadius:urreallydumb?15:0}}>
                                  <TouchableOpacity onPress={() => navigation.navigate('viewP',{userN:index[0]})} style={{flexDirection:'row',}} >
                                        <Image style={{width:40,height:40,borderRadius:100,marginLeft:15,marginTop:5,marginBottom:5,borderWidth:2,borderColor:bgc,alignSelf:'center',marginRight:7}} source={{uri:'https://firebasestorage.googleapis.com/v0/b/pika-4f9d6.appspot.com/o/'+index[3]+'?alt=media&token=63a74aae-9e63-4382-a1fa-9179491c5f16?time='+String(new Date).slice(0,21)}} />
                                        <Text style={{fontFamily:'courierBold',fontSize:16,color:bgc,alignSelf:'center'}} >@{index[0]}</Text>
                                        {index[5]?<Image source={require('./imgs/verified.png')} style={{marginLeft:5,marginBottom:0,alignSelf:'center',height:12,width:12,tintColor:bgc}} />:<View></View>}
                                  </TouchableOpacity>
                                  ...
                                  <Text style={{fontFamily:'cour',fontSize:16,color:bgc,marginLeft:17,marginRight:40,paddingTop:15,width:Wwidth-80}} >{index[1]}</Text>
                                  ...
                                  <View style={{flexDirection:'column',position:'absolute',right:10,bottom:0}}>
                                          <TouchableOpacity style={{alignSelf:'center',flex:1,flexDirection:'column',justifyContent:'center'}} onPress={() => LikeTap(idx)}>
                                                <Image style={{alignSelf:'center',opacity:count[idx],height:32,width:32,tintColor:bgc,position:'absolute'}} source={require('./imgs/like.png')}/>
                                                <Image style={{alignSelf:"center",opacity:1,height:32,width:32,tintColor:bgc,position:'absolute'}} source={require('./imgs/likeB.png')} />
                              ...
                              {/* <View style={{height:5,width:10,alignSelf:'center',backgroundColor:fgc}}></View> */}
                        </View>
                  ...
                  return(
                    <SafeAreaView style={{backgroundColor:bgc,height:'100%'}}>
                        ...
                        <View style={{marginTop:20,marginBottom:10}} >
                          ...
                          <ActivityIndicator size='large' color={fgc} />
                        </ScrollView>
                    </SafeAreaView>
                  )
                }
  ...
  return (
    <NavigationContainer independent={true}>
      <Stack.Navigator initialRouteName="home"  >
        ...
        <Stack.Screen name="comm" component={suckComm} options={({route}) => ({headerTintColor:bgc,title:'comments',headerTitleStyle:{color:bgc,fontFamily:'cour'},headerStyle:{backgroundColor:fgc}})}/>
