...
<KeyboardAvoidingWrapper>
    <View style={{backgroundColor:bgc,height:'100%'}}>
        <View style={{width:'100%', backgroundColor:'#00e8ff',borderBottomLeftRadius:40,borderBottomRightRadius:40,paddingBottom:25, marginBottom:30}}>
            ...
            <TouchableOpacity style={{alignSelf:'center', marginTop:100,backgroundColor:bgc,paddingHorizontal:30,paddingTop:7,borderRadius:150}} onPress={pickImage}>
            <Text style={{color:'#00e8ff', fontSize:40, fontFamily:'cour'}} >+</Text>
            </TouchableOpacity>
            <TouchableOpacity style={{padding:0,marginTop:0}}>
                {!IsUploading ? <Text style={{color:bgc,fontFamily:'cour',fontSize:24,alignSelf:'center'}} onPress={() => Upload_Img(Images[0])}>next</Text>:<ActivityIndicator size='large' color={bgc} />}
            </TouchableOpacity>
            <Text style={{color:bgc,fontFamily:'cour',alignSelf:'center',fontSize:12}}>the first image is your profile picture</Text>
            {
                Images.map((image,index)=>{
                    
                    return (
                        <View key={index} style={{height:500,width:300,alignSelf:'center',marginBottom:10}}>
                            <Image style={{height:500,width:300,borderRadius:30,borderWidth:2,borderColor:bgc}} source={{uri:Images[index]}} />
                            <TouchableOpacity style={{position:'absolute', bottom:27,right:20,backgroundColor:bgc, height:50,width:50,borderRadius:150}} onPress={() => deleteImg(index)}>
