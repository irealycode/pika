...
function suck_chatt({navigation}) {
    ...
    if (FriendsLists[0] || UserChatsList[0]) {
          ...
          <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['#00e8ff']} progressBackgroundColor={bgc}/>}>
            ...
            <View style={{paddingBottom:20}}>
            {
                        UserChatsList.map((index,idx) => {
                            return (
                                <TouchableOpacity key={idx} style={{borderRadius:15,backgroundColor:'#00e8ff',alignSelf:'center',width:'80%',height:100,marginTop:10,flexDirection:'row'}} onPress={() => navigation.navigate('chat1', {Rdata:index,name : '@'+index[0]})}>
                                    ...
                                    <Text style={{fontFamily:'cour',fontSize:16,color:bgc,marginLeft:15,marginTop:33}}>@{index[0]}</Text>
        ...
        return(
            <SafeAreaView style={{backgroundColor:bgc,height:'100%'}}>
            <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['#00e8ff']} progressBackgroundColor={bgc} />}>
                      <ActivityIndicator size='large' color={'#00e8ff'} style={{alignSelf:'center',marginTop:'50%'}} />
            </ScrollView>
          </SafeAreaView>
        )
      }
  ...
  return (
    <NavigationContainer independent={true}>
      <Stack.Navigator initialRouteName="list" >
        ...
        <Stack.Screen name="chat" component={suck_chattt} options={({route}) => ({title: route.params.name,headerTintColor:'#00e8ff',headerTitleStyle:{fontFamily:'cour',fontSize:18},headerStyle:{backgroundColor:bgc} })} />
        <Stack.Screen name="chat1" component={suck_chatttt} options={({route}) => ({title: route.params.name,headerTintColor:'#00e8ff',headerTitleStyle:{fontFamily:'cour',fontSize:18},headerStyle:{backgroundColor:bgc}})} />
      </Stack.Navigator>
    </NavigationContainer>
  );
}
