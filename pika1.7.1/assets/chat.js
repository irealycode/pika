...
export default function suck_chattt({route, navigation}) {
    ...
    const userRef2 = db.collection('users').doc(Rdata[3]);
      ...
      loadTheme()
        ...
        return <GiftedChat messagesContainerStyle={{backgroundColor:bgcc}} showAvatarForEveryMessage={false} renderAvatar={(props) => renderAvatar(props)} renderBubble={(props) => <Bubble {...props} wrapperStyle={{right:{bottom:10,backgroundColor:'#00e8ff',borderWidth:2,borderColor:bgc,},left:{bottom:10,backgroundColor:bgc,borderWidth:2,borderColor:'#00e8ff'}}} textStyle={{left:{color:'#00e8ff',fontFamily:'cour'},right:{color:bgc,fontFamily:'cour'}}} />} renderSend={(props) => <Send {...props} containerStyle={{borderWidth:0,}} textStyle={{color:'#00e8ff',fontFamily:'cour',top:8}}  />} renderInputToolbar={(props) => <InputToolbar {...props}containerStyle={{backgroundColor: bgc,borderTopColor: "#00e8ff",borderTopWidth:2,borderColor: "#00e8ff",borderWidth: 2,padding: 5,borderRadius:10,elevation:0}} textInputStyle={{ color: "#00e8ff",fontFamily:'cour',}} placeholder='message...' />} messages={messages} user={user} onSend={handleSend} />
